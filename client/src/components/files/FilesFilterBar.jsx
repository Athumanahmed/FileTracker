...
import { FILE_STATUS_META, FILE_PRIORITY_META } from "../../utils/fileStatusMeta";

const STATUS_OPTIONS = [{ value: "", label: "All Statuses" }, ...Object.entries(FILE_STATUS_META).map(([value, meta]) => ({ value, label: meta.label }))];
const PRIORITY_OPTIONS = [{ value: "", label: "All Priorities" }, ...Object.entries(FILE_PRIORITY_META).map(([value, meta]) => ({ value, label: meta.label }))];
...
/** All three map 1:1 onto GET /api/v1/files' query params (search, status, priority). */
const FilesFilterBar = ({ search, onSearchChange, status, onStatusChange, priority, onPriorityChange, onReset }) => {
  ...
  const activeFilterCount = [search, status, priority].filter(Boolean).length;
    ...
    <div className="flex flex-col lg:flex-row lg:items-center gap-3">
      <div className="relative flex-1 min-w-55">
        ...
        <input
          ...
          placeholder="Search by file number, tracking number or title..."
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={priority} onChange={(e) => onPriorityChange(e.target.value)} className={selectClassName}>
