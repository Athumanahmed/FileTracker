...
import { User, AtSign, Phone, IdCard, Crown, Loader2, Send, ShieldCheck, RotateCcw } from "lucide-react";
import BaseInput from "../shared/BaseInput";
import UserCreatedSuccessModal from "./UserCreatedSuccessModal";
import { useCreateUser } from "../../hooks/useCreateUser";
import { createDirector } from "../../utils/apiServices";
...
// Mirrors server/validators/user.validation.js's baseUserFieldsRules
// (via createGlobalRoleValidationRules, the rules POST /api/v1/users/directors
// actually runs) field-for-field, so nothing that passes here gets
...
/**
 * Director is a global-scope role (see server/config/organizationalHierarchy.js)
 * -- no department/unit assignment belongs on this form, unlike HOD/Supervisor/
 * Officer.
 */
const CreateDirector = () => {
  ...
  const { mutate: submitCreateDirector, isPending, data: createdResult, reset: resetMutation } =
    useCreateUser(createDirector);

  const onSubmit = (formData) => {
    submitCreateDirector(formData, {
      onError: (error) => {
        const message = error?.response?.data?.message || "Unable to create Director. Please try again.";
        toast.error(message);
      },
    });
  };
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className="flex items-start gap-3 border-b border-gray-100 p-5 sm:p-6">
        ...
        <div>
          <h2 className="font-bold text-gray-900">Municipal Director Registration</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Provide the information needed to create this Director&apos;s account.
          </p>
        </div>
      </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <BaseInput
                    ...
                    value={field.value}
                    onChange={(_, value) => field.onChange(value)}
                    placeholder="e.g. director1@example.com"
