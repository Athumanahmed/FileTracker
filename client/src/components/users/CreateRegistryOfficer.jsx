...
import { createRegistryOfficer } from "../../utils/apiServices";
...
// createGlobalRoleValidationRules, the rules POST /api/v1/users/registry-officers
// actually runs) field-for-field, so nothing that passes here gets rejected
// server-side.
const createRegistryOfficerSchema = z.object({
  firstName: z
    .string()
    .trim()
...
/**
 * Registry Officer is a global-scope role (see
 * server/config/organizationalHierarchy.js) -- same shape as Director,
 * no department/unit assignment belongs on this form.
 */
const CreateRegistryOfficer = () => {
  ...
  const {
    ...
    resolver: zodResolver(createRegistryOfficerSchema),
    defaultValues,
  });

  const { mutate: submitCreateRegistryOfficer, isPending, data: createdResult, reset: resetMutation } =
    useCreateUser(createRegistryOfficer);

  const onSubmit = (formData) => {
    submitCreateRegistryOfficer(formData, {
      onError: (error) => {
        const message =
          error?.response?.data?.message || "Unable to create Registry Officer. Please try again.";
        toast.error(message);
      },
    });
  };
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className="flex items-start gap-3 border-b border-gray-100 p-5 sm:p-6">
        ...
        <div>
          <h2 className="font-bold text-gray-900">Registry Officer Registration</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Provide the information needed to create this Registry Officer&apos;s account.
          </p>
        </div>
      </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <BaseInput
                    ...
                    value={field.value}
                    onChange={(_, value) => field.onChange(value)}
                    placeholder="e.g. registry.test@eftms.test"
...
export default CreateRegistryOfficer;
