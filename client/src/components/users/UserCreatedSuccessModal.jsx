import { useEffect, useState } from "react";
import { createPortal } from "react-dom";
import toast from "react-hot-toast";
import { CheckCircle2, AlertTriangle, Copy, Check, X, RotateCcw, ArrowRight } from "lucide-react";

/**
 * Shown immediately after any "create X" admin form succeeds -- every
 * server/services/user.service.js's createUser call returns a
 * defaultPassword exactly once (never retrievable again after this
 * response), so this is the one place across all Create*.jsx forms that
 * reveals it. Deliberately does NOT close on backdrop click -- an admin
 * accidentally dismissing this before copying the password would mean
 * generating a brand new one via a password reset just to recover.
 */
const UserCreatedSuccessModal = ({ user, defaultPassword, onCreateAnother, onDone }) => {
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === "Escape") onDone();
    };
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText(defaultPassword);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast.error("Couldn't copy automatically -- select and copy the password manually.");
    }
  };

  const displayName = user.fullName || user.username;

  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="user-created-title"
        className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl"
      >
        <button
          type="button"
          onClick={onDone}
          aria-label="Close"
          className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 transition-colors"
        >
          <X size={16} />
        </button>

        <div className="flex flex-col items-center text-center px-6 pt-7 pb-5 border-b border-gray-100">
          <span className="flex h-14 w-14 items-center justify-center rounded-full bg-green-50 text-green-600 mb-3">
            <CheckCircle2 size={28} />
          </span>
          <h2 id="user-created-title" className="text-lg font-bold text-gray-900">
            Account Created Successfully
          </h2>
          <p className="text-sm text-gray-500 mt-1">{displayName}&apos;s account is ready.</p>
        </div>

        <div className="px-6 py-5 space-y-4">
          <div className="flex gap-2.5 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3">
            <AlertTriangle size={16} className="mt-0.5 shrink-0 text-amber-600" />
            <p className="text-xs leading-relaxed text-amber-800">
              This temporary password is shown <strong>only once</strong>. Copy it now and share it
              securely with {displayName} — it cannot be retrieved again after you leave this screen.
            </p>
          </div>

          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
              Temporary Password
            </label>
            <div className="mt-1.5 flex items-center gap-2">
              <code className="flex-1 select-all rounded-xl border border-gray-200 bg-gray-50 px-3.5 py-2.5 font-mono text-sm text-gray-800 overflow-x-auto">
                {defaultPassword}
              </code>
              <button
                type="button"
                onClick={handleCopy}
                aria-label="Copy password"
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border transition-colors ${
                  copied
                    ? "border-green-200 bg-green-50 text-green-600"
                    : "border-gray-200 text-gray-500 hover:bg-gray-50"
                }`}
              >
                {copied ? <Check size={16} /> : <Copy size={16} />}
              </button>
            </div>
          </div>

          <div className="rounded-xl border border-gray-100 divide-y divide-gray-100 text-sm">
            <div className="flex items-center justify-between px-4 py-2.5">
              <span className="text-gray-500">Username</span>
              <span className="font-medium text-gray-800">{user.username}</span>
            </div>
            <div className="flex items-center justify-between px-4 py-2.5">
              <span className="text-gray-500">Email</span>
              <span className="font-medium text-gray-800">{user.email}</span>
            </div>
          </div>
        </div>

        <div className="flex gap-3 px-6 pb-6 pt-1">
          <button
            type="button"
            onClick={onCreateAnother}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors"
          >
            <RotateCcw size={15} />
            Create Another
          </button>
          <button
            type="button"
            onClick={onDone}
            className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-primaryBlue px-4 py-2.5 text-sm font-semibold text-white hover:bg-primaryBlueDark transition-colors"
          >
            Done
            <ArrowRight size={15} />
          </button>
        </div>
      </div>
    </div>,
    document.body,
  );
};

export default UserCreatedSuccessModal;
