...
import { User, AtSign, Phone, IdCard, Archive, Loader2, Send, ShieldCheck, RotateCcw } from "lucide-react";
import BaseInput from "../shared/BaseInput";
import UserCreatedSuccessModal from "./UserCreatedSuccessModal";
import { useCreateUser } from "../../hooks/useCreateUser";
import { createArchiveOfficer } from "../../utils/apiServices";
...
// createGlobalRoleValidationRules, the rules POST /api/v1/users/archive-officers
// actually runs) field-for-field, so nothing that passes here gets rejected
// server-side.
const createArchiveOfficerSchema = z.object({
  firstName: z
    .string()
    .trim()
...
/**
 * Archive Officer is a global-scope role (see
 * server/config/organizationalHierarchy.js) -- same shape as Director/
 * Registry Officer, no department/unit assignment belongs on this form.
 */
const CreateArchiveOfficer = () => {
  ...
  const {
    ...
    resolver: zodResolver(createArchiveOfficerSchema),
    defaultValues,
  });

  const { mutate: submitCreateArchiveOfficer, isPending, data: createdResult, reset: resetMutation } =
    useCreateUser(createArchiveOfficer);

  const onSubmit = (formData) => {
    submitCreateArchiveOfficer(formData, {
      onError: (error) => {
        const message =
          error?.response?.data?.message || "Unable to create Archive Officer. Please try again.";
        toast.error(message);
      },
    });
  };
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div className="flex items-start gap-3 border-b border-gray-100 p-5 sm:p-6">
        ...
        <div>
          <h2 className="font-bold text-gray-900">Archive Officer Registration</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Provide the information needed to create this Archive Officer&apos;s account.
          </p>
        </div>
      </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <BaseInput
                    ...
                    value={field.value}
                    onChange={(_, value) => field.onChange(value)}
                    placeholder="e.g. amina.archive@eftms.test"
...
export default CreateArchiveOfficer;
